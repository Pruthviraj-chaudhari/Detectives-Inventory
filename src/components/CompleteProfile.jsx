...
import { skills, roles, requiredFields } from "../Data";
...
function CompleteProfile() {
  ...
  const { name, email, about, github, linkedin, isProfileComplete } = userData;
  ...
  const selectOptions = skills.map((name) => ({
    label: name,
  ...
  const submitHandler = async () => {
    ...
    if (!isFormValid && !userData.isProfileComplete) {
      toast.error("Please fill in all required fields.");
      return;
    }

    const skills = selectedSkills.map((item) => item.label);
    ...
    const errorMessage = "Error occurred while updating profile";
